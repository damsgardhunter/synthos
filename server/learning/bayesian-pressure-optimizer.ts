...
function rbfKernel1D(x1: number, x2: number, lengthScale: number, signalVar: number): number {
  const d = (x1 - x2) / lengthScale;
  return signalVar * Math.exp(-0.5 * d * d);
}
...
function gpPredict(
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i; j < n; j++) {
      const k = rbfKernel1D(obs[i].pressureGpa, obs[j].pressureGpa, lengthScale, signalVar);
      K[i][j] = k;
      K[j][i] = k;
    }
    K[i][i] += noiseVar;
  ...
  for (let i = 0; i < n; i++) {
    kStar[i] = rbfKernel1D(targetPressure, obs[i].pressureGpa, lengthScale, signalVar);
  }
...
function expectedImprovement(
  ...
  return Number.isFinite(ei) ? Math.max(0, ei) : 0;
}
...
export function optimizePressureForFormula(
  ...
  const finalPred = gpPredict(obs, 0, 30);
  ...
  const optPred = gpPredict(obs, optimalPressure, 30);

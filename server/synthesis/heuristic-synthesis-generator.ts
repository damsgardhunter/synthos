...
function parseNestedFormulaCounts(s: string): Record<string, number> {
  ...
  while (i < s.length) {
    if (s[i] === '(') {
      ...
      const inner = parseNestedFormulaCounts(s.substring(i + 1, j - 1));
      let numStr = '';
...
function parseFormula(formula: string): FormulaBreakdown {
  ...
  const counts = parseNestedFormulaCounts(cleaned);

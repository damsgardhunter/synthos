...
export function EngineControls({ engineState, activeTasks, connected, messages }: EngineControlsProps) {
  ...
  const recentEvents = messages
    .filter((m) => m.type === "log" || m.type === "prediction" || m.type === "insight")
    .slice(-8)
    .reverse();
            ...
            <ScrollArea className="h-48">
              <div className="divide-y divide-border">
                {recentEvents.map((msg, i) => (
                  <div key={i} className="px-4 py-2.5 flex items-start gap-2" data-testid={`live-event-${i}`}>
                    <Zap className="h-3 w-3 text-primary mt-0.5 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <span className="text-xs font-medium">
                        {msg.type === "log" ? msg.data.event : msg.type === "prediction" ? `New prediction: ${msg.data.name}` : `Insight discovered`}
                      </span>
                      <p className="text-xs text-muted-foreground mt-0.5 truncate">
                        {msg.type === "log" ? msg.data.detail : msg.type === "prediction" ? `${msg.data.formula} (${(msg.data.confidence * 100).toFixed(0)}% confidence)` : msg.data.insights?.[0]}
                      </p>
                    </div>

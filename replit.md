...
- **Physics Engine**: Deterministic calculations using a 96-element database, including advanced parameter calculation (DOS, bandwidth, Hubbard U/W, lambda, phonon frequencies, metallicity). Incorporates specialized adjustments for superhydrides, High-Entropy Alloys (HEAs), and unconventional superconductors.
- **Topological Superconductor Detection Engine**: Estimates Z2 invariant, Chern number, and Majorana feasibility based on band structure topology and material patterns.
- **Quantum Pairing Mechanism Simulator (7-Channel)**: Analyzes phonon, spin-fluctuation, orbital-fluctuation, excitonic, CDW, polaronic, and plasmon pairing channels with material-aware weighting.
- **Fermi Surface Reconstruction Engine**: Computes Fermi surface isosurfaces, classifies pockets, and extracts features like `pocket_count` and `nesting_vectors`.
...
- **Advanced Physics Constraints (8-Channel)**: Evaluates Fermi surface nesting, orbital hybridization, Lifshitz transition proximity, quantum critical fluctuation, electronic dimensionality, phonon soft modes, charge transfer energy, and lattice polarizability.
...
- **Materials Genome Representation (256-dim)**: Latent genome vector encoding from 8 physics-derived segments for similarity search and inverse design.
- **Autonomous Theory Discovery Engine**: Uses symbolic regression to discover mathematical relationships from simulation data, with overfitting mitigation and theory knowledge base storage.
- **Multi-Scale Physics Modeling**: Analyzes atomic, electronic, and mesoscopic feature layers and their cross-scale coupling.
- **Self-Improving Physics Models**: Parameterized physics equations (mu_star, phonon_scale, anharmonic_factor, pairing_weight) optimized via Bayesian methods, with continuous learning loops.
...
- **Generator Resource Manager**: Centralized allocation and adaptive rebalancing for 7 candidate generators (RL, inverse_design, BO_exploration, massive_combinatorial, structure_diffusion, motif_diffusion, random_exploration).
- **Fermi Surface Topology Clustering**: Clusters FS feature vectors into 5 archetype clusters (cuprate_cylinder, pnictide_eh_pockets, kagome_flat, hydride_multiband, conventional_3d) for search guidance.
- **Materials Discovery Landscape**: UMAP embedding engine for 256D genome vectors, 3D latent space, and discovery zone detection for targeted exploration.
- **Constraint-Based Physics Solver**: Backward McMillan/Eliashberg solver and 4 additional constraint solvers (DOS, phonon frequency, electron-phonon coupling, charge transfer) for inverse design.
- **Pressure-to-Ambient Pathway Search**: Generates stabilization strategies (substitution, doping) for high-pressure superconductors to achieve ambient pressure stability.
- **Physics Constraint Graph Solver**: Solves all SC physics constraints simultaneously as a graph, propagating parameter ranges.
...
- **Synthesis Variables Parameter Space**: Defines 10 categories of synthesis parameters with 20 numeric and 2 discrete variables.
...
- **Quantum Criticality Detector**: Unified QCP detector formalizing Mott, SDW, CDW, nematic, structural, and orbital-selective detection into a `QuantumCriticalScore`.
- **Discovery Memory System**: Pattern-based learning system that remembers physics patterns leading to high Tc discoveries, used to bias generation.
- **Band Structure Neural Network Surrogate**: GNN-based predictor for band structure features (bandGap, flatBandScore, vhsProximity, nesting, DOS) using crystal structure graphs.
- **Phase Stability Prediction Network**: GNN-based pre-filter for rapid screening of material stability before expensive physics calculations.
...
- **Graph Neural Network Surrogate (CGCNN/MEGNet/M3GNet-style)**: Primary ML predictor (0.6 weight) for formation energy, phonon stability, Tc, confidence, and electron-phonon lambda, with uncertainty estimation.
- **Reinforcement Learning Chemical Space Agent**: REINFORCE policy gradient agent learning optimal elements, stoichiometries, and structures, with physics-aware rewards and experience replay.
- **Bayesian Optimization for Chemical Space Search**: Gaussian Process surrogate with mixed acquisition functions for balanced exploration/exploitation.
- **Crystal Diffusion Generator**: Generates novel crystal structures by refining random atomic positions using physics-based score functions and symmetry enforcement.
...
- **Physics-Aware ML Predictor**: Multi-target prediction with transfer priors and uncertainty estimation, self-reinforcing loop.
...
- **Differentiable Materials Design (Gradient-Based Inverse Physics)**: Computes analytical McMillan gradients and numerical element gradients for Tc, mapping signals to discrete composition actions.
- **8-Pillar SC Optimizer (Multi-Objective Superconductivity Targeting)**: Targets 8 superconductivity pillars (electron-phonon coupling, phonon frequencies, DOS, Fermi nesting, structural motifs, pairing glue strength, electronic instability, hydrogen cage geometry) with adaptive weights and weakness-targeted mutation strategies.
- **Physics-Constrained Generative AI (Rule-Aware Material Creation)**: Enforces charge neutrality, atomic radius compatibility, coordination number limits, and bond stability.
- **Structure-First Design (Structure Diffusion)**: Selects SC structural motifs, fills site roles with element candidates, and evaluates using GB Tc prediction.
...
- **Analytical Physics Estimators**: Provides estimates for Debye temperature, bulk modulus, density, DOS at Fermi level, average phonon frequency, and atomic packing fraction.
...
- **Semantic Insight Deduplication**: Uses OpenAI text-embedding-3-small for semantic deduplication of insights before LLM novelty scoring.
- **Kagome Metals Family**: Generates AV3X5, AV3X4, A2V3X5 stoichiometries and applies specific family filters.
- **Layered Structure Generators**: Creates layered chalcogenides, pnictides, and intercalated layered structures.
- **Mixed-Mechanism Systems**: Targets Fe/Ni/Co/Cu in layered compounds for phonon + magnetic fluctuation superconductivity.
- **Tight-Binding Electronic Structure Model**: Builds H(k) matrix for band structure, Wannier projection, and band topology detection.
- **Flat-Band Detection**: Computes DOS(EF)/avgDOS ratio as a flat-band indicator.

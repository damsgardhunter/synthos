...
- **Lambda-Based Tc Clamp**: Pre-physics safeguard: if Tc > 80K but lambda < 1.5, Tc is penalized (×0.15 for lambda<0.5, ×0.25 for lambda<1.0, ×0.3 for lambda<1.5). Prevents ML hallucination of high-Tc materials.
- **Family-Specific Tc Caps (PHYSICS_VERSION=13)**: Carbides capped at 45K ambient/80K high-pressure, Nitrides 50K/90K, Borides 55K/120K, Oxides 40K/70K. Applied in both `applyAmbientTcCap()` and engine recalculation.
- **Expanded Family Classifier**: 15 families: Hydrides, Cuprates, Heavy Fermions, Pnictides, Chalcogenides, Sulfides, Borides, Carbides, Nitrides, Silicides, Phosphides, Oxides, Intermetallics, Alloys, Other. Reduced "Other" from 74 to ~36.
- **Insight Quality Filter**: `isInsightSpecificEnough()` rejects vague insights ("show varied", "can have different", "tend to be/have/show") and requires quantitative data, specific materials, or clear correlations. Applied in both Phase 3 and Phase 5.
- **Milestone Deduplication**: Pipeline-graduate thresholds restored from existing milestones on restart, preventing repeated "10 candidates fully validated" events.
- **End-to-End Pipeline Pass Rate**: Changed from per-stage average (was 66.7%) to stage-0-entered/stage-4-passed ratio (now ~10.9%). Pipeline stage thresholds tightened: ML filter requires ensemble>0.55 AND xgb>0.3, electronic structure metallicity>0.45 and DOS>0.8, phonon lambda>0.5, Tc>10K, synthesizability>0.55.
- **Family Diversity Metric**: Now counts all unique families across all candidates (up to 500), not just top 50.

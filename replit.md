...
- **XGBoost Bootstrap Ensemble Uncertainty**: 5 bootstrap-trained GBModels (each on 80% sample with replacement) in `gradient-boost.ts`. `gbPredictWithUncertainty(features, formula)` returns `{tcMean, tcStd, normalizedUncertainty, score, perModelPredictions, acquisitionScore, reasoning}`. Ensemble rebuilt on every `retrainXGBoostFromEvaluated()` call. Stats: `getXGBEnsembleStats()`. API: `/api/xgboost/uncertainty/:formula`, `/api/xgboost/ensemble-stats`.
- **Three-Tier Active Learning Selection**: `selectForDFT` in `active-learning.ts` uses three tiers: (1) best predicted Tc (10 slots), (2) highest combined GNN+XGB uncertainty (5 slots), (3) random exploration (remaining slots). Combined uncertainty = 50% GNN + 50% XGB ensemble std. `RankedCandidate` includes `xgbUncertainty` and `selectionTier` fields. Tier breakdown logged each cycle.

...
- **Real DFT Backend (GFN2-xTB)**: Integrates xtb v6.7.1 for quantum-mechanical properties, geometry optimization, and finite-displacement phonon calculations. Formation energy uses isolated single-atom references with correct UHF (unpaired electron) counts for metals; molecular elements (H,N,O,F,Cl) use dimer/2 references. Positive total energies rejected as xTB failures.

...
- **ML Enhancement Session (8 tasks)**:
  ...
  - active-learning.ts: UCB acquisition function with adaptive alpha (max(0.5, 2.0-0.3*retrains)); 20% budget reserved for pure exploration (highest-uncertainty candidates). Full ensemble retraining (all 5 models). Convergence detection: if avg uncertainty drop < 0.1 over 3 consecutive retrains, retrain threshold increased 15→30.
